...
def available_words(list_of_letters):
    ...
    final_list = [word for word in final_list if len(
        word) <= len(list_of_letters)]
...
user_input = input(
    'Please input your set of letters altogether(no separations)')

user_words = scrabble_cheat_list([letter for letter in user_input])
words_formatted = [f'{tuple[0]}: {tuple[1]}' for tuple in user_words]
